...
namespace CLI
{
	...
	inline std::string dblRow(const std::string &content)
	{
		...
		return std::string(CLR_DIVIDER) + BOX_DV + " " + RESET + content + pad + std::string(CLR_DIVIDER) + BOX_DV + RESET;
	}
	inline std::string row(const std::string &content)
	{
		...
		if (vl < avail)
			pad = std::string(avail - vl, ' ');
		return std::string(CLR_DIVIDER) + BOX_V + " " + RESET + content + pad + std::string(CLR_DIVIDER) + BOX_V + RESET;
	}
	...
	// ── Single-line hint progress (updates same line) ────────────────────────
	inline void printHintProgress(const std::string &msg)
	{
		if (Settings::disablePrint)
			return;
		std::cerr << "\r  " << CLR_WARN << SYM_ARROW << " " << RESET << CLR_DIM << msg << RESET << "   " << std::flush;
	}
}
